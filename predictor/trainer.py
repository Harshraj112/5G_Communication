...
from config import (
    ...
    TRANSFORMER_LR, TRANSFORMER_EPOCHS, TRANSFORMER_BATCH,
    PRETRAIN_STEPS, MODEL_WEIGHTS_PATH, SCALER_PATH,
)
...
def train_transformer(
    ...
    if verbose:
        print(f"\n✓ Model saved → {MODEL_WEIGHTS_PATH}")
...
class TrafficPredictor:
    ...
    def __init__(self, model: TrafficTransformer, scaler, device: str = "cpu"):
    ...
    def update(self, demand: dict[str, float]):
        ...
        vec = np.array([demand["eMBB"], demand["URLLC"], demand["mMTC"]], dtype=np.float32)
...
if __name__ == "__main__":
    ...
    records = sim.run(duration=PRETRAIN_STEPS)
