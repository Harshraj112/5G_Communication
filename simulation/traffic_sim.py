...
import numpy as np
from config import (
    NUM_UES, SLICE_RATIOS, TRAFFIC_PROFILES, SIM_DURATION
)
...
def ue_process(env: simpy.Environment, ue_id: int, slice_type: str,
               demand_store: dict, rng: np.random.Generator):
    """
...
class TrafficSimulator:
    ...
    def run(self, duration: int = SIM_DURATION) -> list[dict]:
        ...
        for slice_type, ue_id in self.ue_slices:
            env.process(ue_process(env, ue_id, slice_type,
                                   demand_store, self.rng))
...
class StreamingTrafficSimulator:
    ...
    def _setup(self):
        ...
        for slice_type, ue_id in ue_slices:
            self._env.process(ue_process(self._env, ue_id, slice_type,
                                         self._demand_store, self.rng))
